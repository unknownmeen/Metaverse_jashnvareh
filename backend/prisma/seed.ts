...
async function main() {
  ...
  const festival2 = await prisma.festival.upsert({
    ...
    create: {
      ...
      rulesText: 'این جشنواره برگزار شده است؛ ارسال تصویر جدید ممکن نیست. همچنان می‌توانید تصاویر و نقدها را ببینید و امتیاز ثبت کنید.',
  ...
  const festival3 = await prisma.festival.upsert({
    ...
    create: {
      ...
      rulesText: 'این جریان هنوز در مرحلهٔ قبل از برگزاری است. با آغاز برگزاری، ارسال تصویر فعال می‌شود.',
...
📋 خلاصه داده‌های تست:
  ...
  • جریان‌ها: ۳ جشنواره (قبل از برگزاری، در حال برگزاری، برگزار شده)
