import { ArrowLeft, Loader2, MessageCircle, Pencil, Star, Trash2 } from "lucide-react";
...
import { EditImageModal } from "@/features/streams/edit-image-modal";
import {
  DELETE_IMAGE_MUTATION,
  GET_FESTIVAL_IMAGES_QUERY,
  GET_MY_IMAGES_QUERY,
...
export function ImageCard({ image, subtitle }: ImageCardProps) {
  ...
  const [updateImageMutation] = useMutation(UPDATE_IMAGE_MUTATION);
  ...
  const isAdminManager = currentUser?.role === "ADMIN" || currentUser?.role === "SUPER_ADMIN";
  const isOwner = currentUser?.id === image.userId;
  const canDeleteImage = Boolean(isAdminManager || isOwner);
  const showOwnerHoverActions = Boolean(isOwner);
  const showAdminDeleteOnly = Boolean(isAdminManager && !isOwner);
  const initialCoverIndex = Math.max(image.galleryUrls.findIndex((url) => url === image.url), 0);

  const handleDeleteImageRequest = (event: MouseEvent<HTMLButtonElement>) => {
    event.preventDefault();
    event.stopPropagation();

    if (!canDeleteImage || deletingImage) {
      return;
    }
  ...
  const handleSaveEdit = async (urls: string[], title: string, description: string, coverIndex: number) => {
  ...
  return (
    <>
      ...
      >
        <div className="flex h-full flex-col overflow-hidden rounded-3xl border border-slate-100 bg-white shadow-sm transition-[transform,box-shadow] duration-300 hover:-translate-y-1.5 hover:border-slate-100 hover:shadow-xl hover:shadow-slate-200/60">
          <div className="relative aspect-[4/3] overflow-hidden rounded-t-3xl bg-slate-100">
            {showOwnerHoverActions || showAdminDeleteOnly ? (
              <div className="absolute left-3 top-3 z-10 flex flex-col gap-2 opacity-0 transition-all duration-200 group-hover:opacity-100">
                {showOwnerHoverActions ? (
                  <button
                    type="button"
                    onClick={handleOpenEdit}
                    className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-white/95 bg-white/92 text-slate-500 shadow-[0_12px_30px_rgba(15,23,42,0.14)] backdrop-blur-xl transition-all duration-200 hover:border-white hover:bg-white hover:text-primary-600"
                    title={t("image_card.edit_image")}
                  >
                ...
                <button
                  type="button"
                  onClick={handleDeleteImageRequest}
                  className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-white/95 bg-white/92 text-slate-500 shadow-[0_12px_30px_rgba(15,23,42,0.14)] backdrop-blur-xl transition-all duration-200 hover:border-white hover:bg-white hover:text-red-600"
      ...
      <EditImageModal
        ...
        streamName={image.festival?.name ?? subtitle ?? ""}
        initialTitle={image.title ?? ""}
        initialDescription={image.description ?? ""}
        initialUrls={image.galleryUrls.length > 0 ? image.galleryUrls : [image.url]}
